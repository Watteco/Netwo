...
class SerialSocket extends BluetoothGattCallback {
    ...
    private void onPairingBroadcastReceive(Context context, Intent intent) {
        ...
        switch (intent.getAction()) {
            case BluetoothDevice.ACTION_PAIRING_REQUEST:
                ...
                logs.add("pairing request " + pairingVariant);
            ...
            case BluetoothDevice.ACTION_BOND_STATE_CHANGED:
                ...
                logs.add("bond state " + previousBondState + "->" + bondState);
                break;
            default:
                Log.d(TAG, "unknown broadcast " + intent.getAction());
                logs.add("unknown broadcast " + intent.getAction());
                break;
        }
    }
    ...
    public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
        ...
        if (newState == BluetoothProfile.STATE_CONNECTED) {
            ...
            logs.add("connect status "+status+", discoverServices");
            if (!gatt.discoverServices()){
                logs.add("discoverServices failed");
                onSerialConnectError(new IOException("discoverServices failed"));
            }
        } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
            if (connected){
                logs.add("gatt status" + status);
            ...
            logs.add("unknown connect state "+newState+" "+status);
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        logs.add("\"servicesDiscovered, status \"" + status);
        if (canceled)
            return;
    ...
    private void connectCharacteristics1(BluetoothGatt gatt) {
        ...
        if(delegate==null || readCharacteristic==null || writeCharacteristic==null) {
            for (BluetoothGattService gattService : gatt.getServices()) {
                ...
                logs.add("service "+gattService.getUuid());
                for(BluetoothGattCharacteristic characteristic : gattService.getCharacteristics()){
                    Log.d(TAG, "characteristic "+characteristic.getUuid());
                    logs.add("characteristic "+characteristic.getUuid());
                }
            }
    ...
    private void connectCharacteristics2(BluetoothGatt gatt) {
        ...
        logs.add("request max MTU");
        if (!gatt.requestMtu(MAX_MTU)){
            logs.add("request MTU failed");
    ...
    public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
        ...
        logs.add("mtu size "+mtu+", status="+status);
        if(status ==  BluetoothGatt.GATT_SUCCESS) {
            payloadSize = mtu - 3;
            Log.d(TAG, "payload size "+payloadSize);
            logs.add("payload size "+payloadSize);
        }
    ...
    private void connectCharacteristics3(BluetoothGatt gatt) {
        ...
        if((writeProperties & (BluetoothGattCharacteristic.PROPERTY_WRITE +     // Microbit,HM10-clone have WRITE
            ...
            logs.add("write characteristic not writable");
            onSerialConnectError(new IOException("write characteristic not writable"));

            return;
        }
        if(!gatt.setCharacteristicNotification(readCharacteristic,true)) {
            logs.add("no notification for read characteristic");
        ...
        if(readDescriptor == null) {
            logs.add("no CCCD descriptor for read characteristic");
        ...
        if((readProperties & BluetoothGattCharacteristic.PROPERTY_INDICATE) != 0) {
            ...
            logs.add("enable read indication");
            ...
            logs.add("no indication/notification for read characteristic ("+readProperties+")");
        ...
        logs.add("writing read characteristic descriptor");
        if(!gatt.writeDescriptor(readDescriptor)) {
            logs.add("read characteristic CCCD descriptor not writable");
    ...
    public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
        ...
        if(descriptor.getCharacteristic() == readCharacteristic) {
            ...
            logs.add("writing read characteristic descriptor finished, status="+status);
            if (status != BluetoothGatt.GATT_SUCCESS) {
                logs.add("write descriptor failed");
    ...
    void write(byte[] data) throws IOException {
        ...
        synchronized (writeBuffer) {
            ...
            if(!writePending && writeBuffer.isEmpty() && delegate.canWrite()) {
                ...
                logs.add("write queued, len="+data0.length);
            ...
            if(data.length > payloadSize) {
                for(int i=1; i<(data.length+payloadSize-1)/payloadSize; i++) {
                    ...
                    logs.add("write queued, len="+(to-from));
        ...
        if(data0 != null) {
            ...
            if (!gatt.writeCharacteristic(writeCharacteristic)) {
                ...
                logs.add("write started, len="+data0.length);
    ...
    public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        if(characteristic == writeCharacteristic) { // NOPMD - test object identity
            ...
            logs.add("write finished, status="+status);
            writeNext();
        }
    }
    ...
    private void writeNext() {
        ...
        if(data != null) {
            ...
            if (!gatt.writeCharacteristic(writeCharacteristic)) {
                ...
                logs.add("write started, len="+data.length);
    ...
    private class NrfDelegate extends DeviceDelegate {
        ...
        boolean connectCharacteristics(BluetoothGattService gattService) {
            ...
            if (rw2 != null && rw3 != null) {
                ...
                logs.add("characteristic properties " + rw2prop + "/" + rw3prop);
                if (rw2write && rw3write) {
                    logs.add("multiple write characteristics (" + rw2prop + "/" + rw3prop + ")");
                    ...
                    logs.add("no write characteristic (" + rw2prop + "/" + rw3prop + ")");
    ...
    private class TelitDelegate extends DeviceDelegate {
        ...
        boolean connectCharacteristics(BluetoothGattService gattService) {
            ...
            logs.add("service telit tio 2.0");
            readCredits = 0;
            writeCredits = 0;
            ...
            if (!gatt.setCharacteristicNotification(readCreditsCharacteristic, true)) {
                logs.add("no notification for read credits characteristic");
            ...
            if (readCreditsDescriptor == null) {
                logs.add("no CCCD descriptor for read credits characteristic");
            ...
            if (!gatt.writeDescriptor(readCreditsDescriptor)) {
                logs.add("read credits characteristic CCCD descriptor not writable");
                onSerialConnectError(new IOException("read credits characteristic CCCD descriptor not writable"));
                return false;
            }
            Log.d(TAG, "writing read credits characteristic descriptor");
            logs.add("writing read credits characteristic descriptor");
            return false;
        ...
        void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            if(descriptor.getCharacteristic() == readCreditsCharacteristic) {
                ...
                logs.add("writing read credits characteristic descriptor finished, status=" + status);
                if (status != BluetoothGatt.GATT_SUCCESS) {
                    onSerialConnectError(new IOException("write credits descriptor failed"));
                    logs.add("write credits descriptor failed");
                } else {
                    connectCharacteristics2(gatt);
                }
            }
        ...
        void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            if(characteristic == readCreditsCharacteristic) { // NOPMD - test object identity
                ...
                logs.add("got write credits +"+newCredits+" ="+writeCredits);

                if (!writePending && !writeBuffer.isEmpty()) {
                    Log.d(TAG, "resume blocked write");
                    logs.add("resume blocked write");
                    writeNext();
                }
            }
            if(characteristic == readCharacteristic) { // NOPMD - test object identity
                ...
                logs.add("read, credits=" + readCredits);
            }
        }
        ...
        void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            if(characteristic == writeCharacteristic) { // NOPMD - test object identity
                ...
                logs.add("write finished, credits=" + writeCredits);
            ...
            if(characteristic == writeCreditsCharacteristic) { // NOPMD - test object identity
                ...
                logs.add("write credits finished, status="+status);
            }
        }
